...
def memcheck(test):
    ...
    v = find_valgrind()
    cmd = v+" --tool=memcheck --child-silent-after-fork=yes --leak-check=full --xml=yes --xml-fd=3 --num-callers=50 " + test + " 3>/tmp/.memcheck.tmp"
    #print cmd
    system(cmd)
    out = open("/tmp/.memcheck.tmp").readlines()
